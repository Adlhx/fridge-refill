...
class Product(TimeStamped):
    name=models.CharField(max_length=180); short_name=models.CharField(max_length=100,blank=True); brand=models.CharField(max_length=100,blank=True); variant=models.CharField(max_length=120,blank=True); size=models.CharField(max_length=80,blank=True); package_type=models.CharField(max_length=80,blank=True); barcode=models.CharField(max_length=64,blank=True,db_index=True); sku=models.CharField(max_length=64,blank=True,db_index=True); category=models.CharField(max_length=100,blank=True); product_photo=models.ImageField(upload_to='products/%Y/%m/',blank=True); pack_size=models.CharField(max_length=80,blank=True); needs_verification=models.BooleanField(default=False,db_index=True); active=models.BooleanField(default=True)
...
class FridgeProduct(TimeStamped):
    created_at=models.DateTimeField(auto_now_add=True,null=True,db_index=True); updated_at=models.DateTimeField(auto_now=True,null=True); fridge=models.ForeignKey(Fridge,on_delete=models.CASCADE,related_name='assignments'); product=models.ForeignKey(Product,on_delete=models.PROTECT,related_name='fridge_assignments'); shelf_number=models.PositiveIntegerField(default=1); position=models.PositiveIntegerField(default=0); display_order=models.PositiveIntegerField(default=0); notes=models.TextField(blank=True); normal_capacity=models.PositiveIntegerField(null=True,blank=True); minimum_display_quantity=models.PositiveIntegerField(null=True,blank=True); active=models.BooleanField(default=True)
    class Meta: ordering=['shelf_number','position','display_order','id']; constraints=[models.UniqueConstraint(fields=['fridge','product'],name='unique_fridge_product')]; indexes=[models.Index(fields=['fridge','shelf_number','position'])]
class RefillSession(TimeStamped):
    ...
    store=models.ForeignKey(Store,on_delete=models.PROTECT,related_name='sessions',db_index=True); employee=models.ForeignKey(User,on_delete=models.PROTECT,related_name='refill_sessions',db_index=True); started_at=models.DateTimeField(auto_now_add=True); completed_at=models.DateTimeField(null=True,blank=True); deleted_at=models.DateTimeField(null=True,blank=True,db_index=True); status=models.CharField(max_length=20,choices=Status.choices,default=Status.IN_PROGRESS,db_index=True)
